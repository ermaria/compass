...
import { IconButton, Grid, Card, Typography, Table, TableBody, TableCell, TableContainer, TableRow } from '@mui/material';
import { styled } from "@mui/system"; // Import styled from @mui/system
import LocationOnIcon from '@mui/icons-material/LocationOn'; // Import LocationOn icon
import Link from "next/link";
import OpenInNewIcon from '@mui/icons-material/OpenInNew';
import { canIDeployLiveNow, hasPearlHacksStarted } from '../hasPearlHacksStarted';
...
const EventCardWrapper = styled(Card)(({ theme }) => ({
  ...
  export const PreEventComponent = ({ events }) => {
    const workshops = events.filter(event => event.Type.includes('Workshop'));
    return (
      <Grid container spacing={2}> {/* Grid container with spacing */}
        {workshops.map((event, index) => (
          ...
          <a href={event.Link ? event.Link : '/schedule'} target="_blank"
...
export const DayOfEventComponent = ({ events, selectedTheme }) => {
    const dayOfEvents = events.filter(event => event.Type.includes('Day-of'));
              ...
              <Table aria-label={`Events on ${day}`} style={{ borderSpacing: '0 10px', borderCollapse: 'separate' }}>
                <TableBody>
                  {groupedEvents[day].map((event, index) => (
                    <TableRow key={index}>
                      <TableCell style={{ width: '20%', fontWeight: 'bold', borderBottom: '2px solid white', color: selectedTheme == "light" ? "#1876bd" : "#F5EDC9"}}><Typography variant='body1'> {`${event["Start Time"]}`}  {event["End Time"] ? `- ${event["End Time"]}` : ""} </Typography></TableCell>
